...
lines(x = 1:P,
      y = exp(-(d*2)*(1:P)), # change the d value to see how function shape changes
      ylim=c(0,1),
      col = 2,
      type = 'b')
...
for(p in 1:P){
  ...
  if(E[p]>0) {
    ...
    dest <- sample(x = 1:P, size = E[p], replace=TRUE, prob=disp_mat[p,])
...
# Metacommunity model:
# Simulate dynamics according to the classic metacommunity paradigms (see Fig 2 in paper) and contrast dynamics visually

# Contrast and compare other parts of the parameter space in Figure 2 (i.e., not just the edges of the classic paradigms)
# e.g., change one or both of the dispersal parameters (reminder a controls EMIGRATION, or the distance decay parameter, which controls IMMIGRATION), or
# contrast communities with broad versus narrow niches, or
# contrast communities with different interaction matrices, or
